...
async function clearDataSession(session, token) {
    try {
        const config = {
            headers: {
        ...
        const response = await axios.post(`https://grantosegurosapimanagement-production.up.railway.app/api/${session}/THISISMYSECURETOKEN/clear-session-data`, {}, config);
...
module.exports = clearDataSession;